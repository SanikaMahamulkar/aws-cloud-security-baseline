...
**SSM Session Manager registration — resolved.** Despite a correctly configured IAM instance profile, all three required VPC interface endpoints reporting `available`, and security group rules verified via CLI to correctly permit HTTPS between the instance and the endpoints, the EC2 target instance initially did not register with Systems Manager. Root cause, found via `aws ec2 get-console-output`: the Amazon Linux 2023 AMI does not ship with the `amazon-ssm-agent` service pre-installed, so the original `user_data` script's `systemctl enable/start` calls were operating on a non-existent unit. All previously-verified network and IAM configuration was genuinely correct throughout; the fault was at the OS/agent layer. Fixed by installing the official agent RPM in `user_data` before enabling the service, and confirmed resolved via a real `aws ssm send-command` execution returning genuine output. The practical lesson: a fully-verified network and IAM path can still mask a simple missing-dependency issue, and reading raw console boot logs — rather than continuing to re-verify already-confirmed networking — was what surfaced the actual cause.
